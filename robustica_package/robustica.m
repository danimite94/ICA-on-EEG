...
%
%         ksign   : source kurtosis signs (one element per source or empty);
...
%                   termination test is not used if tol < 0, so that the algorithm runs the maximum number
%                   of iterations, except if contrast is constant (only one source left)
%
...
thmu = 2*tol;       % mu_opt-based termination test, to finish if converged to constant value of mu_opt
...
while keep_going
    ...
    [g, mu_opt] = kurt_gradient_optstep(w, Z, signkurt, P);  
         
    % update extracting vector
    wn = w + mu_opt*g;      % update 

    wn = P*(wn/norm(wn));   % normalize extracting vector and project if required
    ...
    if th < tol2 | it >= max_it | mu_opt == 0 
        % finish when extracting vector converges, too many iterations have been run or 
        % if optimal step-size is zero
       keep_going = 0;     
   end % if th < tol2 ...
...
if do_reg
    W(:,k) = [zeros(n-dimobs, 1); w];  % zero-padding to account for dimensionality reduction in regression
else
